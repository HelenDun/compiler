...
grammar_test_runnable:
	$(foreach file, $(wildcard $(DIR_TESTS_RUNNABLE)/*(?!$(PPV_FILE)).ul), echo; echo $(file) ; java Compiler $(file);)
grammar_test_parsable:
	$(foreach file, $(wildcard $(DIR_TESTS_PARSABLE)/*(?!$(PPV_FILE)).ul), echo; echo $(file) ; java Compiler $(file);)
grammar_test_invalid:
	$(foreach file, $(wildcard $(DIR_TESTS_INVALID)/*(?!$(PPV_FILE)).ul), echo; echo $(file) ; java Compiler $(file);)

# java Compiler ./tests/runnable/some_file.ul -ppv
ppv_test: ppv_test_runnable ppv_test_parsable ppv_test_ultimate
ppv_test_runnable:
	$(foreach file, $(wildcard $(DIR_TESTS_RUNNABLE)/*(?!$(PPV_FILE)).ul), echo; echo $(file) ; java Compiler $(file) -ppv;)
ppv_test_parsable:
	$(foreach file, $(wildcard $(DIR_TESTS_PARSABLE)/*(?!$(PPV_FILE)).ul), echo; echo $(file) ; java Compiler $(file) -ppv;)
ppv_test_ultimate:
...
tcv_test_runnable:
	$(foreach file, $(wildcard $(DIR_TESTS_RUNNABLE)/*(?!$(PPV_FILE)).ul), echo; echo $(file) ; java Compiler $(file) -tcv;)
tcv_test_parsable:
	$(foreach file, $(wildcard $(DIR_TESTS_PARSABLE)/*(?!$(PPV_FILE)).ul), echo; echo $(file) ; java Compiler $(file) -tcv;)
